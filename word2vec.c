...
/*
 *model update incremently for text format ---OK
 *model update incremently for binary format ---ok 
 *Increment Training ---ok
 */
...
char train_file[MAX_STRING], output_file[MAX_STRING],
		model_output_file[MAX_STRING], model_input_file[MAX_STRING];
...
real *syn0, *syn1, *syn1neg, *expTable, *syn1neg_old, *syn0_old;
;
clock_t start;
...
void SortVocab() {
	...
	for (a = 0; a < size; a++) {
		...
		if (vocab[a].cn < min_count) {
			vocab_size--;
			free(vocab[vocab_size].word);   //free鎺夎word
		} else {
			// Hash will be re-computed, as after the sorting it is not actual
			hash = GetWordHash(vocab[a].word);
			while (vocab_hash[hash] != -1)
				hash = (hash + 1) % vocab_hash_size;
			vocab_hash[hash] = a;   //瀛樺偍璇ord鍦╲ocab閲岄潰鐨勪笅鏍�
...
//鑺傜偣涓暟锛歷ocab_size * 2 + 1
//鐖朵翰鑺傜偣涓暟锛歷ocab_size * 2 + 1
//count:鍚勪釜word鐨刬d缂栧彿
...
void InitNetOnline() {
    ...
    char* vocab_word_point = (char *) malloc((long long) max_w * sizeof(char));

	a = posix_memalign((void **) &syn0_old, 128,
	...
	if (negative > 0) {
		a = posix_memalign((void **) &syn1neg_old, 128,
				(long long) vocab_size_old * layer1_size * sizeof(real));
...
void SaveModel() {
	...
	FILE *fo = fopen(model_output_file, "wb");
    
    char* foo_str=model_output_file;
    strcat(foo_str,"11");

    FILE *foo = fopen(foo_str, "wb");
