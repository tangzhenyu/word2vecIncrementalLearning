...
// synonyms and antonyms @tangzhenyu 2016-1-8
...
// relation from freebase @tangzhenyu 2016-1-11
...
int relation_num = 0;
// parameters vector @tangzhenyu 2016-1-19
...
void ReadVocab() {
  ...
  if (debug_mode > 0) {
    printf("Vocab size: %lld\n", vocab_size);
    printf("Words in train file: %lld\n", train_words);
  }/* 为了测试暂时注释 @tangzhenyu 2016-01-08*/
  fin = fopen(train_file, "rb");
  if (fin == NULL) {
    printf("ERROR: training data file not found!\n");
    exit(1);
